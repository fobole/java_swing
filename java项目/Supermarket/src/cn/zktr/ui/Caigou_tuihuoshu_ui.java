package cn.zktr.ui;

import java.awt.BorderLayout;
import java.awt.FlowLayout;

import javax.swing.JButton;
import javax.swing.JDialog;
import javax.swing.JPanel;
import javax.swing.JPopupMenu;
import javax.swing.border.EmptyBorder;

import org.jvnet.substance.SubstanceLookAndFeel;

import cn.zktr.entity.Cgoods_jiujia;

import javax.swing.JLabel;
import javax.swing.JOptionPane;

import java.awt.Color;
import javax.swing.JComboBox;
import javax.swing.JTextField;
import javax.swing.SwingUtilities;
import javax.swing.UIManager;
import javax.swing.UnsupportedLookAndFeelException;

import java.awt.Font;
import javax.swing.JScrollPane;
import javax.swing.JTable;
import java.awt.event.ActionListener;
import java.awt.event.ActionEvent;
import java.awt.Toolkit;
import javax.swing.event.CaretListener;
import javax.swing.event.CaretEvent;

public class Caigou_tuihuoshu_ui extends JDialog {

	private final JPanel contentPanel = new JPanel();
	private JTextField textField_1;
	private JTable table;
	
	
	//创建一个父窗口对象
	Caigou_jiujiatui_ui gui;
	/**
	 *修改数量界面
	 */
//	public static void main(String[] args) {
//		try {
//			Caigou_gaishu_ui dialog = new Caigou_gaishu_ui();
//			dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
//			dialog.setVisible(true);
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}

	/**
	 * Create the dialog.
	 */
	public Caigou_tuihuoshu_ui(JFrameInterface jframe) {
		//将传回的父窗口进行接收
		this.gui=(Caigou_jiujiatui_ui) jframe.getJFrame();
		setTitle("商品信息");
		setIconImage(Toolkit.getDefaultToolkit().getImage(Caigou_geshu_ui.class.getResource("/img/logo544.png")));
//		try {
//			UIManager.setLookAndFeel(new SubstanceLookAndFeel());
//		} catch (UnsupportedLookAndFeelException e1) {
//			// TODO Auto-generated catch block
//			e1.printStackTrace();
//		}
		//子页面关闭才能动主页面
						setModal(true);
						//不能改变大小
						setResizable(false);

		


		setBounds(100, 100, 643, 438);
		//居中
		setLocationRelativeTo(null);
		getContentPane().setLayout(new BorderLayout());
		contentPanel.setBorder(new EmptyBorder(5, 5, 5, 5));
		getContentPane().add(contentPanel, BorderLayout.CENTER);
		contentPanel.setLayout(null);
		
		JPanel panel = new JPanel();
		panel.setBounds(0, 0, 671, 397);
		contentPanel.add(panel);
		panel.setLayout(null);
		
		JLabel label = new JLabel("商品信息");
		label.setFont(new Font("微软雅黑", Font.BOLD, 30));
		label.setBounds(257, 0, 122, 41);
		panel.add(label);
		
		JPanel panel_1 = new JPanel();
		panel_1.setBounds(10, 47, 616, 340);
		panel.add(panel_1);
		panel_1.setLayout(null);
		
		JLabel label_1 = new JLabel("商品编号：");
		label_1.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_1.setBounds(53, 21, 110, 32);
		panel_1.add(label_1);
		
		JLabel label_2 = new JLabel("商品名称：");
		label_2.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_2.setBounds(331, 21, 103, 32);
		panel_1.add(label_2);
		
		JLabel label_3 = new JLabel(gui.cj.getGid()+"");
		label_3.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_3.setForeground(new Color(0, 255, 0));
		label_3.setBounds(190, 21, 131, 26);
		panel_1.add(label_3);
		
		JLabel label_5 = new JLabel("基本单位：");
		label_5.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_5.setBounds(331, 159, 103, 27);
		panel_1.add(label_5);
		
		JLabel label_4 = new JLabel(gui.cj.getGspec());
		label_4.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		label_4.setBounds(445, 159, 118, 27);
		panel_1.add(label_4);
		
		JLabel label_6 = new JLabel("当前库存：");
		label_6.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_6.setBounds(53, 159, 110, 27);
		panel_1.add(label_6);
		
		JLabel label_9 = new JLabel(gui.cj.getGname());
		label_9.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		label_9.setBounds(444, 24, 124, 26);
		panel_1.add(label_9);
		
		JLabel label_12 = new JLabel(gui.cj.getStshu()+"");
		label_12.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		label_12.setBounds(190, 159, 54, 27);
		panel_1.add(label_12);
		
		JPanel panel_2 = new JPanel();
		panel_2.setBounds(0, 223, 651, 60);
		panel_1.add(panel_2);
		panel_2.setLayout(null);
		
		JLabel label_15 = new JLabel("数   量：");
		label_15.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_15.setBounds(180, 18, 80, 31);
		panel_2.add(label_15);
		
		textField_1 = new JTextField(1+"");
		textField_1.addCaretListener(new CaretListener() {
			public void caretUpdate(CaretEvent e) {
//				JTextField textField_1 = (JTextField) e.getSource();// 获得触发事件的 JTextField
//				String text = textField_1.getText();
//				if (text.length() == 0) {
//					return;
//					}
//					char ch = text.charAt(text.length() - 1);
//					if(!(ch >='0'&&ch<='9')) { // 中文，最常用的范围是 U+4E00～U+9FA5，也有使用 U+4E00～ U+9FFF 的，但目前 U+9FA6～U+9FFF 之间的字符还属于空码，暂时还未定义，但不能保证以后不会被定义
//					JOptionPane.showMessageDialog(textField_1,"只能输入数字","提示",JOptionPane.INFORMATION_MESSAGE);
//					SwingUtilities.invokeLater(new Runnable() {
//						
//						@Override
//						public void run() {
//							// 去掉 JTextField 中的末尾字符
//							textField_1.setText(text.substring(0,text.length()-1));
//							
//						}
//					});
//				}
			}
		});
		textField_1.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		textField_1.setBounds(282, 17, 101, 31);
		panel_2.add(textField_1);
		textField_1.setColumns(10);
		
		JButton button_1 = new JButton("取  消");
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Caigou_tuihuoshu_ui.this.dispose();
			}
		});
		button_1.setFont(new Font("微软雅黑", Font.BOLD, 25));
		button_1.setBounds(51, 305, 146, 32);
		panel_1.add(button_1);
		
		JLabel label_7 = new JLabel("进      价：");
		label_7.setFont(new Font("微软雅黑", Font.BOLD, 20));
		label_7.setBounds(53, 82, 110, 31);
		panel_1.add(label_7);
		
		JLabel label_10 = new JLabel(gui.cj.getGbid()+"");
		label_10.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		label_10.setBounds(190, 82, 118, 31);
		panel_1.add(label_10);
		
		JLabel lblNewLabel = new JLabel("售      价：");
		lblNewLabel.setFont(new Font("微软雅黑", Font.BOLD, 20));
		lblNewLabel.setBounds(331, 82, 103, 26);
		panel_1.add(lblNewLabel);
		
		JLabel lblNewLabel_1 = new JLabel(gui.cj.getGprice()+"");
		lblNewLabel_1.setFont(new Font("微软雅黑", Font.PLAIN, 20));
		lblNewLabel_1.setBounds(445, 82, 100, 31);
		panel_1.add(lblNewLabel_1);
		
		JButton button = new JButton("确  定");
		button.setBounds(395, 305, 131, 32);
		panel_1.add(button);
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int gid=Integer.parseInt(label_3.getText());
				String gname=label_9.getText();
				String gspec=label_4.getText();
				double gbid=Double.parseDouble(label_10.getText());
				double gprice=Double.parseDouble(lblNewLabel_1.getText());
				
				if(textField_1.getText().equals(null) || textField_1.getText().equals("")) {
					JOptionPane.showMessageDialog(null, "输入不合法");
					return;
				}
				char ch = textField_1.getText().charAt(textField_1.getText().length() - 1);
				if(!(ch >='0'&&ch<='9')) {
					JOptionPane.showMessageDialog(null, "输入不合法");
					return;
				}
				int	stshu=Integer.parseInt(textField_1.getText());
				if(stshu>Integer.parseInt(label_12.getText())) {
					JOptionPane.showMessageDialog(null, "退货不能大于库存");
					return;
				}
//				Cgoods_jiujia g=new Cgoods_jiujia(gid, gname, gbid, gprice, gspec, stshu);
				Cgoods_jiujia g=gui.cj;
				g.setStshu(stshu);
				for (Cgoods_jiujia l : gui.list2) {
					if(l.getGid()==gid) {
						l.setStshu(l.getStshu()+stshu);
						try {
							gui.updateTable2();
						} catch (Exception e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
						Caigou_tuihuoshu_ui.this.dispose();
						return;
					}
				}
				gui.list2.add(g);
				//新增成功，调用父窗口的刷新方法，并关闭当前窗口
				try {
					gui.updateTable2();
					Caigou_tuihuoshu_ui.this.dispose();
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				
			}
		});
		button.setFont(new Font("微软雅黑", Font.BOLD, 25));
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 396, 641, 56);
		contentPanel.add(scrollPane);
		
		table = new JTable();
		scrollPane.setViewportView(table);
		setVisible(true);
	}
}
