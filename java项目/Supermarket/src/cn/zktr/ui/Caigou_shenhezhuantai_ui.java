...
public class Caigou_shenhezhuantai_ui extends JDialog {
...
//	}
	 ...
	 * Create the dialog.
	 */
	public Caigou_shenhezhuantai_ui(JFrameInterface jframe) {
		this.shen=(Caiguo_cai_ui) jframe.getJFrame();
...
//						System.out.println("审核");
							...
							//库存出入库表（come）
							Cgoods_caigou come=new Cgoods_caigou(podd, 0, pdate, pagent,0, pdate);
							
							//2.从表需要list集合中的数据，可以直接将glist传入过去
							try {
								//由于有很多业务，这里必须经过biz业务类处理
								cbiz.insertCaigou(shen.listcai, cai,come);
							} catch (Exception e1) {
								// TODO Auto-generated catch block
								e1.printStackTrace();
							}
									shen.listcai.clear();
									shen.lblNewLabel_2.setText("");
									shen.textField_4_1.setText("");
									 try {
										...
										Caigou_shenhezhuantai_ui.this.dispose();
									} catch (Exception e) {
										// TODO Auto-generated catch block
										e.printStackTrace();
									}
...
//						System.out.println("未审核"); 
						...
						//库存出入库表（come）
						Cgoods_caigou come=new Cgoods_caigou(podd, 0, pdate, pagent,1, pdate);
						
						//2.从表需要list集合中的数据，可以直接将glist传入过去
						try {
							//由于有很多业务，这里必须经过biz业务类处理
							cbiz.insertCaigou2(shen.listcai, cai,come);
						} catch (Exception e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
								...
								shen.textField_4_1.setText("");
								 try {
									...
									//刷新表格
									//新增成功，调用父窗口的刷新方法，并关闭当前窗口
									JOptionPane.showMessageDialog(null, "采购商品，未审核");
									Caigou_shenhezhuantai_ui.this.dispose();
								} catch (Exception e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
