...
public class Caigou_jiujiatui_ui extends JDialog {
...
//	            @Override
//	            public void run() {
//	            	Caigou_jiujia_ui frame = new Caigou_jiujia_ui();
...
//	    }
	 ...
	 * Create the dialog.
	 */
	public Caigou_jiujiatui_ui(JFrameInterface jframe) {
		this.cai=(Caigou_tui_ui) jframe.getJFrame();
...
//							Caigou_geshu_ui c=new Caigou_geshu_ui();
					...
					btnf_3.setBounds(540, 105, 150, 33);
					panel_3.add(btnf_3);
				}
			}
			{
				JPanel panel_4 = new JPanel();
				panel_4.setBounds(0, 146, 741, 59);
				panel.add(panel_4);
...
//							cl_panel_5.show(panel_5, "name_233127670889600");
				...
				button.setBounds(10, 10, 167, 37);
				panel_4.add(button);
			}
			
			panel_5 = new JPanel();
			panel_5.setBounds(0, 203, 741, 546);
			panel.add(panel_5);
			panel_5.setLayout(null);
			
			JPanel panel_2 = new JPanel();
			panel_2.setBounds(0, 0, 750, 546);
			panel_5.add(panel_2);
			panel_2.setLayout(null);
			{
				 scrollPane = new JScrollPane();
				scrollPane.setBounds(0, 0, 740, 546);
				panel_2.add(scrollPane);
				{
								 ...
								 if (e.getClickCount() == 2) {
									//先判断是否选中行
										if(table_1_1.getSelectedRow()<0){
											...
											//打开新增窗口,将当前父窗口传到子窗口中
											Caigou_tuihuoshu_ui add=new Caigou_tuihuoshu_ui(new JFrameInterface() {
												@Override
												public JDialog getJFrame() {
													// TODO Auto-generated method stub
													return Caigou_jiujiatui_ui.this;
												}
											});
										}
								 }
				        ...
				        table_1_1.setDefaultRenderer(Object.class, render1);
				        //线条颜色
//				        table_1.setGridColor(Color.red);
		...
		{
			JPanel panel_2 = new JPanel();
			panel_2.setBounds(771, 0, 642, 749);
			contentPanel.add(panel_2);
			panel_2.setLayout(null);
			{
				JLabel label = new JLabel("所选商品");
				label.setFont(new Font("微软雅黑", Font.BOLD, 25));
				label.setBounds(10, 10, 131, 42);
				panel_2.add(label);
			}
			{
				scrollPane_1 = new JScrollPane();
				scrollPane_1.setBounds(10, 52, 622, 616);
				...
				btnf.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if(table_2.getSelectedRow()<0){
							...
							//通过行获取该行中的列
//							int gid=list2.get(row).getGid();
//							String gname=(String) table_2.getValueAt(row, 0);
//							double gbid=(double) table_2.getValueAt(row, 1);
//							String gspec=(String) table_2.getValueAt(row, 2);
//							int stshu=(int) table_2.getValueAt(row, 3);
							//将属性封装到个公共的商品对象中
...
//							=new Cgoods_jiujia(gid, gname, gbid, list2.get(row).getGprice(), gspec,list2.get(row).getStshu());
//							System.out.println(cj.toString());
							//打开新增窗口,将当前父窗口传到子窗口中
							Caigou_tuihuoshu_ui2 add=new Caigou_tuihuoshu_ui2(new JFrameInterface() {
								@Override
								public JDialog getJFrame() {
									// TODO Auto-generated method stub
									return Caigou_jiujiatui_ui.this;
			...
			}
			{
				...
				btndel.setBounds(198, 690, 103, 38);
				panel_2.add(btndel);
			}
			{
...
//						//总金额
//						double he=0;
//						for (Cgoods_jiujia cg : cai.listcai) {
//							 he+=(cg.getGbid()*cg.getStshu());
//							 
//						}
						...
						Caigou_jiujiatui_ui.this.dispose();
					}
				});
				btnf_1.setFont(new Font("微软雅黑", Font.BOLD, 20));
				btnf_1.setBounds(359, 690, 95, 38);
				panel_2.add(btnf_1);
			}
			{
				...
				btnf_2.setBounds(502, 690, 103, 38);
				panel_2.add(btnf_2);
			}
		}
		setModal(true);
...
//		cai//获取下拉框选中的值
		if(cai.comboBox.getSelectedItem().equals("全部")) {
			Caigou_gongyingshang s1=(Caigou_gongyingshang) cai.comboBox_1.getSelectedItem();
			int sid=s1.getSid();
			if(textField.getText().equals(null) || textField.getText().equals("")){
				list=gdao.gys_cha(sid);
			}else{
				list=gdao.gys_cha(sid, textField.getText());
			}
			return list;
		}else {
			Caigou_gongyingshang s1=(Caigou_gongyingshang) cai.comboBox_1.getSelectedItem();
			int sid=s1.getSid();
			C_kucun_biandong c1=(C_kucun_biandong) cai.comboBox.getSelectedItem();
			int cid=c1.getCid();
			if(textField.getText().equals(null) || textField.getText().equals("")){
				list=gdao.gys_cha(sid, cid);
			}else{
				list=gdao.gys_cha(sid, cid, textField.getText());
			}
			//2.如果下拉框中有值，则带条件查询
			return list;
		}
	...
	public DefaultTableModel getModel2() throws Exception{
		//创建表头
		Object[] head={"商品名称","进价","单位","数量","库存","总金额"};
		DefaultTableModel tm=null;
		...
		if(list2.size()>0){
			...
			for (int i = 0; i < body.length; i++) {
				...
				try {
					...
					list2.get(i).setXpshu(k.get(0).getSthu());
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
