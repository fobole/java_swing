...
public class Caigou_shenhe_ui extends JDialog {
...
//	}
	 ...
	 * Create the dialog.
	 */
	public Caigou_shenhe_ui(JFrameInterface j) {
		this.sdan=(Caiguo_cai_ui) j.getJFrame();
		//子页面关闭才能动主页面
		setModal(true);
		//不能改变大小
		setResizable(false);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(sdan.table_1_1.getSelectedRow()<0){
					...
					try {
						d.updatezt(podd);
						cbiz.insertzt(sdan.list2);
						sdan.list2.clear();
						sdan.updateTable();
						sdan.updateTable2();
						Caigou_shenhe_ui.this.dispose();
					} catch (Exception e1) {
